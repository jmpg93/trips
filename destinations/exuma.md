...
<script>
  var map = L.map('map').setView([24.17, -76.44], 10);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '© OpenStreetMap'
  }).addTo(map);

  var points = [
    [24.172, -76.443], // 1. Staniel Cay
    [24.180, -76.448], // 2. Thunderball Grotto
    [24.262, -76.512], // 3. Compass Cay
    [24.195, -76.452], // 4. Big Major Cay
    [23.619, -75.922]  // 5. Georgetown (Sur)
  ];
  ...
  createLabel(1, points[0]).addTo(map).bindPopup('1. Staniel Cay (Base)');
  createLabel(2, points[1]).addTo(map).bindPopup('2. Thunderball Grotto');
  createLabel(3, points[2]).addTo(map).bindPopup('3. Compass Cay (Tiburones)');
  createLabel(4, points[3]).addTo(map).bindPopup('4. Big Major Cay (Pig Beach)');
