...
<script>
  var map = L.map('map').setView([31.5, -6.5], 6);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '© OpenStreetMap'
  }).addTo(map);

  var points = [
    [31.63, -7.99], // 1. Marrakech
    [31.37, -6.12], // 2. Valle del Dades
    [31.12, -4.01], // 3. Merzouga (Sáhara)
    [30.93, -6.91], // 4. Ouarzazate
    [31.51, -9.77]  // 5. Essaouira
  ];
  ...
  createLabel(1, points[0]).addTo(map).bindPopup('1. Marrakech (UNESCO)');
  createLabel(2, points[1]).addTo(map).bindPopup('2. Atlas / Valle del Dades');
  createLabel(3, points[2]).addTo(map).bindPopup('3. Sáhara (Erg Chebbi)');
  createLabel(4, points[3]).addTo(map).bindPopup('4. Ait Ben Haddou');
  createLabel(5, points[4]).addTo(map).bindPopup('5. Essaouira (UNESCO)');
